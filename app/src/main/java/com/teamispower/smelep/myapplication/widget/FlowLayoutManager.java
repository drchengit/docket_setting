...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if(BuildConfig.DEBUG)  Log.d(TAG, "onLayoutChildren");
        totalHeight = 0;
        int cuLineTop = top;
        //当前行使用的宽度
        int cuLineWidth = 0;
        ...
        for (int i = 0; i < getItemCount(); i++) {
            if(BuildConfig.DEBUG)  Log.d(TAG, "index:" + i);
      ...
      if(BuildConfig.DEBUG)  Log.d(TAG, "onLayoutChildren totalHeight:" + totalHeight);
